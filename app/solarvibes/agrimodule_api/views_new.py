...
########################                      Blueprint
#############################################################################################################################
agrimodule_api = Blueprint('agrimodule_api', __name__)
...
def get():
    return  jsonify({'Welcome':'This is the Solarvibes API design for the agrimodules to interact with Solarvibes servers!'})


#############################################################################################################################
########################                      0 - CHECK
#############################################################################################################################
@agrimodule_api.route('/check', methods = ['GET'])
@agrimodule_api.route('/check/<identifier>', methods = ['GET'])
# @login_required
def check(identifier = None):


    # VALIDATION
    if not identifier:
        return jsonify({'message':'agrimodule do not exist!'}), 400
...
# @agrimodule_api.route('/register/<identifier>/<mac>', methods = ['GET'])
def register():
    if request.method == 'POST':
        ...
        res_msg, res_http_status = agrimodule_validation(identifier)
        if not res_http_status == 200:
            print(res_msg)
            print(res_http_status)
            return jsonify({'message' : res_msg}), res_http_status
        ###################################
        ###################################
        ...
        if not mac_registered:
            ...
            agrimodule_reg = AgrimoduleList.query.filter_by(identifier = identifier).first_or_404()
            agrimodule_reg.has_agrimodule_registered = True

            agrimodule = Agrimodule.query.filter_by(identifier = identifier).first()
            if not agrimodule:
                return jsonify(dict(message = 'The agrimodule must be registered first- Set your agrimodule system first before turning the agrimodule on'))
